"""
Script for fitting spectral lines in astronomical data using PyMC. This script processes spectral data, identifies emission lines, and fits them using Bayesian inference methods. It supports multiprocessing and allows user-defined inputs to customize the fitting process.
...
Inputs:
    ...
    - ntune: Tuning steps for MCMC.
    - ncores: CPU cores for multiprocessing.

Check example input files for the required format (sample_spec.csv).

Main Workflow:
1. Parse user inputs and set up multiprocessing.
2. Process spectral lines and organize them into sets.
3. Load and preprocess spectral data for each object and grating.
4. Identify emission lines and create fitting windows.
5. Fit spectral lines using PyMC, handling unresolved lines and starting from previous fits if available.
6. Save results, including best-fit parameters, goodness-of-fit metrics, and posterior distributions.
7. Generate diagnostic plots: posterior traces, corner plots, and best-fit models.
8. Log skipped files and clean up temporary files.

Outputs:
- Best-fit parameters and uncertainties (FITS files).
- Diagnostic plots (PNG files).
- Log of skipped files (text file).

Dependencies: Requires `config.py`, `config_lines.py`, and libraries like `arviz`, `pymc`, and `matplotlib`. Temporary files generated by PyTensor are cleaned up after execution.
"""
...
#== User CL inputs:
obj_id_user = sys.argv[1] # PROG_ID from sample_spec.csv
try:
    obj_id_user = float(obj_id_user)
except ValueError:
    pass
nsteps = int(sys.argv[2]) # number of steps per walker
ntune = int(sys.argv[3]) # number of tuning steps
ncores = int(sys.argv[4]) # number of CPU cores
...
def get_line_wavel(w1, w2, lines_dict=[], exclude=[]):
    ...
    lines_dict : dict, optional
        A dictionary: keys - line names (str), values - wavelengths (float or array-like).
    exclude : list, optional
        A list of line names (str) to exclude from the results.
    Returns:
    --------
    names : list
        A list of line names (str) in the specified wavelength range.
    waves : list
        A list of lists, where each inner list contains the wavelengths (float) of the corresponding line name.
    """
...
def mode_in_hdi(samples, bw_fct=2, hdi_prob=0.68, multimodal=False):
    ...
    -----------
    samples : array-like
        Data for which the mode and HDI are to be calculated.
    bw_fct : float, optional
        Bandwidth factor for the kernel density estimation (KDE).
    hdi_prob : float, optional
        The probability mass to be contained within the HDI.
    multimodal : bool, optional
        If True, allows for the possibility of a multimodal distribution and calculates the HDI around the tallest peak.
    Returns:
    --------
    numpy.ndarray
    ...
    for grating, group_grat in df_sample.groupby('grating'):
        for obj_id, group_obj in group_grat.groupby('PROG_ID'):
            for idx_row, row in group_obj.iterrows():
                ...
                sample_info = {
                    'fnames': group_obj.file.values, 
                    'roots': group_obj.root.values, 
                    'idxs': group_obj.index.values, 
                    'obj_ids': group_obj.PROG_ID.values,
                    'z_input': group_obj.z.values,
                }
